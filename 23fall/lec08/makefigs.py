...
def plot_forward(axs, X, Y, W1, B, W2, k, E=None, H=None):
    ...
    if E is None:
        E = np.inner(W1.T,X)
    plot_e_or_h(axs[1], E, 'Excitation $\\mathbf{z}_1=\\mathbf{W}_1\\mathbf{x}$')
    axs[1].set_ylim([-1.1,1.1])    
    if H is None:
        H = sigmoid(E)
    plot_e_or_h(axs[2], H,'')
    axs[2].plot([-0.5,len(H)-0.5],[0.5,0.5],'k--')
    axs[2].set_ylim([-0.1,1.1])
    axs[2].set_xlabel('Activation $\mathbf{a}_1=\sigma(\mathbf{z}_1)$',fontsize=18)
    plot_ydata(axs[3], Y, ymin, ymax)
    a = np.zeros((2,k+1))
    a[:,0] = B
    a[:,1:(k+1)] = W2[:,0:k] * np.tile(np.heaviside(E[0:k],1),[2,1])
    S = arrowseq(axs[3], a)
    axs[3].set_title('$\\mathbf{g}(\\mathbf{x})=\\mathbf{b}_2+...+\\mathbf{w}_{2,:,%d}a_{1,%d}$'%(k+1,k+1))
    return(E,H, S)
...
for k in range(Xi.shape[1]):
    ...
    if k > 0:
        ...
        axs[3].set_title('$\\mathbf{g}(\\mathbf{x})$')
...
axs[0].set_title('Dashed=Hyperplanes $\\perp$ to rows $\\mathbf{w}_{1,j,:}$',fontsize=18)
patch = FancyArrowPatch(Yi[:,datum],Yhat,mutation_scale=10)
patch.set_color('red')
axs[3].add_patch(patch)
axs[3].set_title('Error: $\\mathbf{e}=\\mathbf{g}(\\mathbf{x})-\\mathbf{y}$',fontsize=18)
...
nupdates=5
# number of video frames to compute per gradient update
nframes=5

for k in range(nupdates):
    for t in np.arange(nframes)/nframes:
        E,H,S = plot_forward(axs, Xi[:,datum], Yi[:,datum], W1r, B2r, W2r, 5, E=E, H=H)
        axs[3].add_patch(FancyArrowPatch(Yi[:,datum],Yhat,mutation_scale=10,color='red'))
        ...
        W2t = S[:,k] + t*dW2[:,k]
        axs[0].set_title("$\\frac{\\partial\\mathcal{L}}{\\partial\\mathbf{w}_{1,%d,:}}=-\\mathbf{x}\\frac{\\partial\\mathcal{L}}{\\partial z_{1,%d}}$"%(k+1,k+1),fontsize=18)
        axs[3].set_title("$\\frac{\\partial\\mathcal{L}}{\\partial\\mathbf{w}_{2,:,%d}}=-a_{1,%d}\\mathbf{e}$"%(k+1,k+1))
        fig.savefig('exp/nntrain%d.png'%(nframes*k+int(nframes*t)))
