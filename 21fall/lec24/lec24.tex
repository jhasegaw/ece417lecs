...
\begin{frame}
  \begin{columns}
    ...
    \begin{block}{Exponential Decay}
      \centerline{\includegraphics[width=1.95in]{exp/Plot-exponential-decay.png}}
...
\begin{frame}
  ...
  %~~\epsilon_c[t]=\frac{\partial {\mathcal L}}{\partial c[t]},~~\epsilon_i[t]=\frac{\partial {\mathcal L}}{\partial i[t]},~~\epsilon_o[t]=\frac{\partial {\mathcal L}}{\partial o[t]},~~\epsilon_f[t]=\frac{\partial {\mathcal L}}{\partial f[t]}\]
\end{frame}
...
%  \begin{align*}
%    \frac{d{\mathcal L}}{dh[t]}&=
%    \frac{\partial {\mathcal L}}{\partial h[t]}+
%    \frac{d{\mathcal L}}{di[t+1]}\frac{\partial i[t+1]}{\partial h[t]}+
%    \frac{d{\mathcal L}}{do[t+1]}\frac{\partial o[t+1]}{\partial h[t]}\\
%    &+\frac{d{\mathcal L}}{df[t+1]}\frac{\partial f[t+1]}{\partial h[t]}+
%    \frac{d{\mathcal L}}{dc[t+1]}\frac{\partial c[t+1]}{\partial h[t]}
%  \end{align*}
%  The rule for $c[t]$ is a bit simpler, because $\partial {\mathcal L}/\partial
%  c[t]=0$, so we don't need to include it:
%  \[
%  \frac{d{\mathcal L}}{dc[t]}=\frac{d{\mathcal L}}{dh[t]}\frac{\partial h[t]}{\partial c[t]}+
%  \frac{d{\mathcal L}}{dc[t+1]}\frac{\partial c[t+1]}{\partial c[t]}
%  \]
%\end{frame}
%\begin{frame}
%  \frametitle{Back-Prop Through Time}
%
%  If we define $\delta_h[t]=d{\mathcal L}/dh[t]$, and so on, then we have
%  \begin{align*}
...
%  \begin{align*}
%    \delta_o[t]=\frac{d{\mathcal L}}{do[t]}&=
%    \frac{d{\mathcal L}}{dh[t]}\frac{\partial h[t]}{\partial o[t]}=\delta_h[t]c[t]\\
%    \delta_i[t]=\frac{d{\mathcal L}}{di[t]}&=
%    \frac{d{\mathcal L}}{dc[t]}\frac{\partial c[t]}{\partial i[t]}=
%    \delta_c[t]\sigma_h\left(w_cx[t]+u_ch[t-1]+b_c\right)\\
%    \delta_f[t]=\frac{d{\mathcal L}}{df[t]}&=
%    \frac{d{\mathcal L}}{dc[t]}\frac{\partial c[t]}{\partial f[t]}=\delta_c[t]c[t-1]
%  \end{align*}
%\end{frame}
%
